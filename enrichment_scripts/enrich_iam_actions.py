#!/usr/bin/env python3
"""
Script to enrich AWS IAM registry JSON with official AWS action descriptions.
"""
import json
import sys
import os
...
def enrich_iam_registry(registry_file: str, descriptions_file: str, output_file: str) -> None:
    ...
    if 'iam' in registry and 'actions' in registry['iam']:
        for action in registry['iam']['actions']:
            actions_processed += 1
...
if __name__ == "__main__":
    ...
    registry_file = str(root / "data/aws_iam_registry_iam.json")
    descriptions_file = str(root / "data/iam_actions.json")
    output_file = str(root / "enriched_data/aws_iam_registry_iam_enriched.json")

    os.makedirs(root / "enriched_data", exist_ok=True)
    enrich_iam_registry(registry_file, descriptions_file, output_file)