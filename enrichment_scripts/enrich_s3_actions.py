#!/usr/bin/env python3
"""
Script to enrich AWS IAM S3 registry JSON with official AWS action descriptions.
"""
import json
import sys
import os
...
def enrich_s3_registry(registry_file: str, descriptions_file: str, output_file: str) -> None:
    ...
    if 's3' in registry and 'actions' in registry['s3']:
        for action in registry['s3']['actions']:
            actions_processed += 1
...
if __name__ == "__main__":
    ...
    registry_file = str(root / "data/aws_iam_registry_s3.json")
    descriptions_file = str(root / "data/s3_actions.json")
    output_file = str(root / "enriched_data/aws_iam_registry_s3_enriched.json")

    os.makedirs(root / "enriched_data", exist_ok=True)
    enrich_s3_registry(registry_file, descriptions_file, output_file)
