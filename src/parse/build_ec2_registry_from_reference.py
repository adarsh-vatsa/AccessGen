...
from src.fetch.ec2_reference import fetch_ec2_reference_json, EC2_REF_URL
...
class EC2Registry:
    ...
    service_prefix: str               # "ec2"
    page_url: str                     # EC2_REF_URL
...
OUT = Path("data/aws_iam_registry_ec2.json")
REPORT = Path("data/build_reports/ec2_registry_report.json")
REPORT.parent.mkdir(parents=True, exist_ok=True)

def build_ec2_registry_from_reference(force: bool=False) -> dict:
    ref = fetch_ec2_reference_json(force=force)
    ...
    for i, rdef in enumerate(ref.get("Resources", [])):
        ...
        resource_defs.append(ResourceTypeDef(
            ...
            condition_keys=[],
            source=SourceRef(url=EC2_REF_URL, table="resource_types", row_index=i)
        ))
    ...
    for i, act in enumerate(ref.get("Actions", [])):
        ...
        for r in act.get("Resources", []) or []:
            ...
            rtypes.append(ResourceTypeRef(
                ...
                source=SourceRef(url=EC2_REF_URL, table="actions", row_index=i)
            ))
    ...
    OUT.write_text(json.dumps({"ec2": to_jsonable(reg)}, indent=2), encoding="utf-8")
    report = {
        "service": "ec2",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    rep = build_ec2_registry_from_reference(force=args.force)
