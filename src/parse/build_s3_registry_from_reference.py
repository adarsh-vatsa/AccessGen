...
from src.fetch.s3_reference import fetch_s3_reference_json, S3_REF_URL

# --- Data contracts (compatible with your previous S3-only MVP) ---
...
OUT = Path("data/aws_iam_registry_s3.json")
REPORT = Path("data/build_reports/s3_registry_report.json")
REPORT.parent.mkdir(parents=True, exist_ok=True)

def build_s3_registry_from_reference(force: bool=False) -> dict:
    ref = fetch_s3_reference_json(force=force)

    # Top-level fields (see AWS doc example)
    version = ref.get("Version", "")
    # The Service Reference uses "Name": "s3" at top. Friendly name is not included.
    service_prefix = "s3"
    ...
    for i, rdef in enumerate(ref.get("Resources", [])):
        ...
        # Keep a single template for MVP (first format); you can store the full list later if needed
        arn_template = _first_or_empty(arn_formats)
        # Some services may list resource-scoped keys here; S3 often doesn't, so default to []
        # The published spec doesn't currently surface per-resource condition keys separately,
        # so we leave empty for now and let action-level keys carry most of the weight.
        resource_defs.append(ResourceTypeDef(
            type=rname,
            arn_template=arn_template,
            condition_keys=[],
            source=SourceRef(url=S3_REF_URL, table="resource_types", row_index=i)
        ))
    ...
    for i, act in enumerate(ref.get("Actions", [])):
        ...
        for r in act.get("Resources", []) or []:
            ...
            rtypes.append(ResourceTypeRef(
                ...
                source=SourceRef(url=S3_REF_URL, table="actions", row_index=i)
            ))
    ...
    OUT.write_text(json.dumps({"s3": to_jsonable(reg)}, indent=2), encoding="utf-8")
    report = {
        "service": "s3",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    rep = build_s3_registry_from_reference(force=args.force)
