#!/usr/bin/env python3
"""
Build IAM Actions Pinecone Indexes
Creates dense and sparse indexes for IAM actions with Gemini embeddings and BM25
"""
import argparse
import json
import logging
...
DEFAULT_DENSE_INDEX = "iam-actions-dense-v2"
DEFAULT_SPARSE_INDEX = "iam-actions-sparse-v2"
DEFAULT_DIM = 3072
...
class IAMIndexBuilder:
    ...
    def load_actions(self, input_path: str) -> List[Dict[str, Any]]:
        """Load IAM actions from enriched extras JSON"""
        with open(input_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        
        actions = data["iam"]["actions"]
        ...
        logger.info(f"Loaded {len(actions)} IAM actions")
        return actions
    ...
    def build_indexes(self, input_file: str):
        """Main build process"""
        logger.info("=== Building IAM indexes ===")
        ...
        encoder_path = Path(__file__).parent / "bm25_encoder_iam.pkl"
        ...
        logger.info("=== IAM indexes built successfully ===")
...
def main():
    parser = argparse.ArgumentParser(description="Build IAM Actions Pinecone indexes")
    parser.add_argument(
        "--input",
